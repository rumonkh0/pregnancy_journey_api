...
// @access    Private
exports.getAllReply = asyncHandler(async (req, res, next) => {
  ...
  res.json({ success: true, message: "Found replies", data: comments });
});
...
exports.getReply = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(200)
    .json({ success: true, message: "Reply found", data: comment });
});
...
exports.createReply = asyncHandler(async (req, res, next) => {
  ...
  commentData.comment_id = req.params.commentId;
  const comment = await Comment.create(commentData);
  res
    .status(201)
    .json({ success: true, message: "Reply created", data: comment });
});
